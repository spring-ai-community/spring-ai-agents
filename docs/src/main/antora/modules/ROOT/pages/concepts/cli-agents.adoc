...
== CLI Agents vs Annotation-Based Tools

Understanding the fundamental difference between CLI agents and traditional annotation-based agent frameworks:

[cols="1h,2,2"]
|===
|Aspect |Annotation-Based (@Tool) |CLI Agents

|**Action Scope**
|Predefined Java methods only
|Any bash command (`grep`, `sed`, `git`, `mvn`, etc.)

|**Flexibility**
|Developer must code every tool
|Agent can use any command-line tool

|**Workflow**
|Back-and-forth LLM tool calls
|Autonomous bash execution loop
...
|**Scaffolding**
|Heavy (routing, orchestration)
|Minimal (just goal + workspace)
...
**Key insight** (Noam Brown, OpenAI): Before reasoning models, annotation-based scaffolding was necessary. After reasoning models, "you just give the model the question without any sort of scaffolding and it just does it."

[NOTE]
====
Spring AI supports **both approaches**:

* **@Tool annotations + MCP**: For structured, controlled tool calling (https://spring.io/blog/2025/03/17/model-context-protocol-mcp-in-spring-ai[Spring AI MCP support])
* **CLI agents**: For autonomous, bash-driven execution (Spring AI Agents library)

Different tools for different needs. Choose based on your use case.
====
