...
----
// Good example
public class ClaudeAgentModel implements AgentModel<AgentRequest, AgentResponse> {
     ...
     */
    public ClaudeAgentModel(ClaudeAgentClient client, ClaudeAgentOptions options) {
...
* **Integration Tests**: End with `IT` (e.g., `ClaudeAgentClientIT`)
...
- Add timeout option to ClaudeAgentOptions
- Implement timeout handling in ClaudeAgentClient
...
class ClaudeQuerySmokeTest {
    ...
    @Test
    @EnabledIf("org.springaicommunity.agents.claude.sdk.integration.ClaudeCliDiscovery#isClaudeCliAvailable")
