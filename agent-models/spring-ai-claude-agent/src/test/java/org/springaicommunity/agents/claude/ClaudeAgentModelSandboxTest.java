...
/**
 * Tests for ClaudeAgentModel sandbox integration using dependency injection pattern.
 *
 ...
 */
class ClaudeAgentModelSandboxTest {
	...
	void testAgentModelCentricPattern() throws Exception {
		...
		when(mockClaudeAgentClient.buildCommand(anyString(), any(CLIOptions.class))).thenReturn(expectedCommand);
		...
		when(mockClaudeAgentClient.parseResult(anyString(), any(CLIOptions.class))).thenReturn(queryResult);
		...
		verify(mockClaudeAgentClient).buildCommand(anyString(), any(CLIOptions.class));
		...
		verify(mockClaudeAgentClient).parseResult(eq("{\"result\": \"Task completed successfully\"}"),
	...
	void testSpringStyleConstructorInjection() {
		...
		ClaudeAgentModel model = new ClaudeAgentModel(mockClaudeAgentClient, defaultOptions, mockSandbox);
